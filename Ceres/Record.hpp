...
struct vec2{
    double x;
    double y;
    vec2(double _x, double _y, double _z){
        x = _x;
        y = _y;
    }
    ...
    double& operator[](int i){
        switch(i){
            case 0: return x;
            case 1: return y;
            default: throw "IndexError: index not in range(0, 2)";
        }
    }
};
...
void Record<Ty>::display(){
    for(size_t i = 0; i < storage.size(); ++i){
        printf("Storage[%lu]: %lf, %lf\n", i, storage[i].x, storage[i].y);
    }
    printf("\n");
}
...
template<class Ty>
void Record<Ty>::input(double x, double y){
    contain.emplace_back(Ty(x, y));      //输出容器内加入
}
