...
class Primtree:
    def __init__(self, surface):
        ...
        self.colors={0:DGREY, 1:BLACK, 11:OLIVER, 12:GREY, 13:RED, 2:BLUE, 10:STEEL, 3:PUR, -255:ORAN}
        self.cover=0
    ...
    def setPool(self, radius:int):
        ...
        while self.getChar(x, y) != 12:
            x = random.randint(5, 60)
            y = random.randint(5, 38)
        self.pointSpread(x, y, 12, 2, radius)
    ...
    # 可删除
    def colorMap(self, tag):
        if tag in {0, 1, 2, 3, 8, 10, 11, 12, 13}:
            return self.colors[tag]
        elif tag in range(-254, 0):
            return YELLOW
        else:
            return ORAN
    ...
    def draw(self):
        for x in range(0, 64):
            for y in range(0, 42):
                pygame.draw.rect(self.screen, self.colorMap(self.getChar(x, y)), (30+x*13, 30+y*13, 12, 12), 0)
                sur=self.getChar2(x, y)
                if sur>0:
                    pygame.draw.rect(self.screen, self.colorMap(sur), (30 + x * 13, 30 + y * 13, 12, 12),
                                     0)

if __name__=="__main__":
    pygame.init()
    screen=pygame.display.set_mode((1000, 620), 0, 32)
    pygame.display.set_caption("Test for random prim tree")
