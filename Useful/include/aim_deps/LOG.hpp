...
const std::string DEFAULT_PATH = "/home/sentinel/ROSWorkspace/Autoaim/src/serial_com/log/";
...
public:
    ...
    /**
     * @brief 构造时指定文件名与路径以及构造模式 @overload
    ...
    /** @param @ref LOG @overload */
    void init(const std::string path, const std::string name, const START_UP_MODE mode = REAL_TIME_W);
public:
    /**
...
void LOG::log(const STR str, const TYPETAG tag){
    ...
    if(tag != NOHEAD && tag != INSERT && tag != NONETAG) tmp_s << TAGMAP[tag] << getSystemTime();//生成标准头
    tmp_s << str;                                                              //内容
    if(tag == ISOLATE) tmp_s << std::endl;                                     //是否隔离显示
...
void LOG::log(const T& firstArg, const types&... args){
    if(sizeof...(args) > 0){
        ...
        tmp_s << firstArg << tmp_s.str() << std::endl;
        if(mode == MSG_QUEUE) msg_que.emplace_back(tmp_s.str());                   //string入队
        else out << tmp_s.str();                                                    //写入文件
