...
public:
	...
	/// @param ptr 浮点数指针 从AimDistance中传入 修改AimDistance的一个参数
	void findPossible(float * const ptr = nullptr);						//找到图上所有可能的灯条						
...
private:	
	 ...
	 */
	void contourProcess(
		const std::vector<std::vector<cv::Point> >& ct, 
		int start, int step,
	 ...
	 * @param valid 灯条是否有效
	 */
	void doubleThresh(const std::vector<cv::Point>& ct,
