...
void ArmorPlate::drawArmorPlates(cv::Mat &src, 
    ...
    for (size_t i = 0; i< tar_list.size(); ++i) {
        if(tar_list[i].armor_number != -1 && tar_list[i].valid){   //有意义的数字
            ...
            if(tar_list[i].center != aim_deps::NULLPOINT2f){
...
cv::Point2f ArmorPlate::momentCenter(const cv::Mat &src, const cv::Point2f &offset){
    cv::Mat dst;
    cv::cvtColor(src, dst, cv::COLOR_BGR2GRAY);
	cv::Moments mo = cv::moments(dst);
...
void ArmorPlate::convolution(const cv::Mat &src){
	for(int i = 1; i < src.cols - 1; ++i){
		for(int j = 1; j < src.rows - 1; ++j){
			...
			for(int x = -1; x < 2; ++x){
				for(int y = -1; y < 2; ++y){
					contain.push_back(src.at<cv::Vec3b>(j + y, i + x)[0]);
					contain.push_back(src.at<cv::Vec3b>(j + y, i + x)[1]);
...
void ArmorPlate::postProcess(std::vector<aim_deps::Armor> &tar_list){
    aim_deps::Armor &arm = tar_list.front();
    if(tar_list.size() >= 2){
        std::sort(tar_list.begin(), tar_list.end(), angleCmp);
        arm = tar_list.front();
    }
    else if(tar_list.size() == 0){
        return;
    }
    if( !arm.valid ) return;
    if(center_cnt < 7){             // 当刚开始执行时，需要计算出装甲板的中心位置（取7次成功的匹配中心求平均）
        cv::Point2f center = (momentCenter(arm.left_light.data, arm.left_light.offset) + 
            momentCenter(arm.right_light.data, arm.right_light.offset)) / 2;
        m_ctr += center;
        arm.center = center;
        ++center_cnt;
        printf("This center: (%f, %f)\n", center.x, center.y);
    }
    else{
        if(center_cnt == 7){        // 第七次执行时, 需要做一次中心位置的平均
            m_ctr /= 7;
            center_cnt = 8;
            printf("This center: (%f, %f)\n", m_ctr.x, m_ctr.y);
        }
        cv::Point2f rough(0.0, 0.0);
        for(int i = 0; i < 4; ++i){
            rough += arm.vertex[i];
        }
        rough /= 4;
        cv::Point2f diff = rough - m_ctr;
        if(std::abs(diff.x) > 3.0 || std::abs(diff.y) > 3.0){       // 可能发生错误匹配
            arm.valid = false;
            return;
        }
        arm.center = m_ctr;
