...
int main(int argc, char* argv[])
{
    ...
    std::string outputVideoPath = "/home/sentinel/cv_";
    std::string outPath = outputVideoPath + std::string(argv[1]) + std::string(".avi");
    
    cv::Size sWH = cv::Size(1440, 1080);
	cv::VideoWriter outputVideo;
	outputVideo.open(outPath, CV_FOURCC('D', 'I', 'V', 'X'), 60.0, sWH);		
	bool record_judge = false, low = false;
    cm::CameraCtl ctl;
    ctl.startGrabbing();
    ctl.setExposureTime(140);
    int cnt = 0;
    while (1) {
